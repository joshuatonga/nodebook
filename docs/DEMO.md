...
- Open a blank Nodebook workspace in a WebMCP-enabled browser.
- Open the browser’s agent surface.
- Keep the Nodebook canvas visible while the agent calls tools.
...
- Exclude an unwanted feature and enter a rationale.
- Add one original feature with **Add node** and rename it.
- Accept the remaining proposed scope.
- Lock one feature, then ask the agent to change it and show the skipped locked ID.
